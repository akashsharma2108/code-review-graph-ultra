...
```bash
pip install code-review-graph-ultra                     # 或: pipx install code-review-graph-ultra
...
```bash
pip install "code-review-graph-ultra[embeddings]"          # 本地向量嵌入 (sentence-transformers)
pip install "code-review-graph-ultra[google-embeddings]"   # Google Gemini 嵌入
pip install "code-review-graph-ultra[communities]"         # 社区检测 (igraph)
pip install "code-review-graph-ultra[enrichment]"          # Python 调用解析增强 (Jedi)
pip install "code-review-graph-ultra[eval]"                # 评估基准测试 (matplotlib)
pip install "code-review-graph-ultra[wiki]"                # 使用 LLM 摘要生成 Wiki (ollama)
pip install "code-review-graph-ultra[all]"                 # 所有可选依赖
```
