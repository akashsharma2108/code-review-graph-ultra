...
```bash
pip install code-review-graph-ultra                     # 또는: pipx install code-review-graph-ultra
...
```bash
pip install "code-review-graph-ultra[embeddings]"          # 로컬 벡터 임베딩 (sentence-transformers)
pip install "code-review-graph-ultra[google-embeddings]"   # Google Gemini 임베딩
pip install "code-review-graph-ultra[communities]"         # 커뮤니티 감지 (igraph)
pip install "code-review-graph-ultra[enrichment]"          # Python 호출 해결 보강 (Jedi)
pip install "code-review-graph-ultra[eval]"                # 평가 벤치마크 (matplotlib)
pip install "code-review-graph-ultra[wiki]"                # LLM 요약 위키 생성 (ollama)
pip install "code-review-graph-ultra[all]"                 # 모든 선택적 의존성
```
